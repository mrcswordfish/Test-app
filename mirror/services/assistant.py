...
class VoiceAssistant:
    ...
    def __init__(
        ...
        self.recognizer, self._sr = self._init_recognizer()
    ...
    def _init_tts_engine(self) -> Callable[[str], None]:
        try:
            ...
            missing = exc.name
            raise RuntimeError(
                f"Missing audio package '{missing}'. Install dependencies from requirements.txt or run with --no-voice."
            ) from exc
    ...
    def _init_recognizer(self) -> tuple["sr.Recognizer", "sr"]:
        try:
            ...
            if missing == "aifc":
                raise RuntimeError(
                    "The Python standard library module 'aifc' is missing. Use the standard CPython build (e.g., 'sudo apt install python3 python3-venv python3-pip') or run with --no-voice to skip microphone features."
                ) from exc
            raise RuntimeError(
                f"Missing dependency '{missing}'. Install dependencies from requirements.txt or run with --no-voice."
            ) from exc
