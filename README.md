...
## Install from GitHub on Raspberry Pi
The snippet below clones this repo and completes the essential setup in one pass. Replace `<YOUR_GITHUB_USER>` if you have a fork.

```bash
sudo apt update && sudo apt install -y git python3-venv python3-pip portaudio19-dev libespeak1 ffmpeg mpv yt-dlp python3-gpiozero
git clone https://github.com/<YOUR_GITHUB_USER>/mirror.git
cd mirror
python3 -m venv .venv && source .venv/bin/activate
pip install --upgrade pip
pip install -r requirements.txt
pip install -e .
cp .env.example .env && nano .env  # fill in your keys and preferences
python -m mirror
```

If you see `ModuleNotFoundError: No module named 'mirror'`, double-check that your virtualenv is activated (`source .venv/bin/activate`) and that `pip install -e .` succeeded.

### Fix `ModuleNotFoundError: No module named 'aifc'`
This error means the standard Python audio module is missing (common on slim/embedded Python builds or Python 3.13 previews). On Raspberry Pi OS, install the full CPython runtime before reinstalling dependencies:

```bash
sudo apt update && sudo apt install -y python3 python3-venv python3-pip
# recreate the virtualenv after installing the full runtime
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt
pip install -e .
```

If you still hit the error, run `python - <<'PY'\nimport aifc\nPY` to confirm `aifc` imports cleanly. As a last resort, you can start the mirror without voice support using `python -m mirror --no-voice`.

## Copy the repo to Raspberry Pi without Git
If you already have the code on another machine and just need it on the Pi, use one of these approaches:

- **Secure copy (recommended)**: From your laptop/desktop (with SSH enabled on the Pi), run:
  ```bash
  # From the directory containing this repository on your computer
  scp -r mirror pi@raspberrypi.local:~/mirror
  # or with rsync for faster, resumable transfers
  rsync -avh --delete --exclude .venv ./ pi@raspberrypi.local:~/mirror
  ```
  Then SSH into the Pi and continue with the virtualenv/dependency steps in the sections below.

- **Download the GitHub ZIP directly on the Pi** (no Git required):
  ```bash
  sudo apt update && sudo apt install -y unzip
  wget https://github.com/<YOUR_GITHUB_USER>/mirror/archive/refs/heads/main.zip
  unzip main.zip && mv mirror-main mirror
  cd mirror
  ```

- **USB drive**: Copy the repo folder to a USB stick, plug it into the Pi (`/media/pi/<drive-name>` on Raspberry Pi OS), and move it into your home directory:
  ```bash
  cp -r /media/pi/<drive-name>/mirror ~/mirror
  ```

After copying, create/activate the virtualenv and install dependencies as shown in **Install from GitHub on Raspberry Pi** or **Full installation guide**.
